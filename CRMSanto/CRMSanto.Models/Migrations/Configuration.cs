        ...
        public void SeedProducten(ApplicationDbContext context)
        {
            using (StreamReader sr = new StreamReader(pathProducten))
            {
                ...
                while (line != null)
                {
                    ...
                    p.ProductNr = Convert.ToInt32((line.Split(',')[0]));
                    p.Naam = Convert.ToString(line.Split(',')[1]);
                    p.Inhoud = Int16.Parse(line.Split(',')[2]);
                    p.AankoopPrijs = Decimal.Parse(line.Split(',')[3]);
                    p.VerkoopPrijs = Decimal.Parse(line.Split(',')[4]);
                    p.MinStock = 1;
